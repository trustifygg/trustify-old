...
export async function execute(interaction: ChatInputCommandInteraction) {
	...
	const averageRating = await reviewModel.aggregate([{ $group: { _id: null, avg: { $avg: '$rating' } } }]);
	...
	const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
		new ButtonBuilder().setLabel('Invite Me').setURL('https://trustify.gg/invite').setStyle(ButtonStyle.Link),
		new ButtonBuilder().setLabel('Support Server').setURL('https://trustify.gg/support').setStyle(ButtonStyle.Link)
	);
