...
export const authenticate = async (c: Context, next: Next) => {
	try {
		...
		if (session?.user?.userId) {
			const user = await userModel.findOne({ userId: session.user.userId }).select('+accessToken +refreshToken');
		...
		try {
			...
			const user = await userModel.findOne({ userId: decoded.userId }).select('+accessToken +refreshToken');
			...
			try {
				...
				if (decodedWithoutVerification.payload.exp && decodedWithoutVerification.payload.exp < now) {
